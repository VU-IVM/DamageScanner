...
from vector import _match_raster_to_vector
...
def RasterScanner(exposure_file,
                  hazard_file,
                  curve_path,
                  maxdam_path,
                  lu_crs=28992,
    ...
    if isinstance(hazard_file, PurePath):
        if (hazard_file.parts[-1].endswith('.tif') | hazard_file.parts[-1].endswith('.tiff')):      
            ...
            hazard,landuse = _match_raster_to_vector(hazard,landuse,lu_crs,haz_crs,resolution,hazard_col)  
        ...
        # Match raster to vector
        hazard,landuse = _match_raster_to_vector(hazard_file,landuse,lu_crs,haz_crs,resolution,hazard_col)  
