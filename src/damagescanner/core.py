...
class DamageScanner(object):
    ...
    def prepare(self, assessment_type, data_path, curves = pd.DataFrame(), maxdam = dict(),osm_pbf=False,**kwargs):
        ...
        ## set up the assessment type and data path
    ...
    def single_event(self,hazard_type='',hazard_file='',return_period='',save_output=False,vector_landuse=True,**kwargs):
        ...
        # check if the assessment type is set up
        if not hasattr(self, 'assessment_type'):
            raise ImportError('ERROR: Please run .prepare() first to set up the core of the assessment')

        # check if the hazard data is ready and available
        if hazard_file != '':
            ...
            if hazard_type == '':
                ...
                if hazard_type == 'flood':
                    hazard_characteristics = ['inun','flood','fluvial','pluvial','coastal']
                ...
                hazard_maps = [x for x in self.hazard_data if any(y in str(x) for y in hazard_characteristics)]
                
                if return_period != '':
                    self.hazard_map = [x for x in hazard_maps if return_period in str(x)][0]
