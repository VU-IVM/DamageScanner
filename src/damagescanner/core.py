...
def RasterScanner(landuse_map,inun_map,curve_path,maxdam_path,centimeters=False,save=False,**kwargs):
    ...
    if landuse.shape != inundation.shape:
        warnings.warn("WARNING: landuse and inundation maps are not the same shape. Let's fix this first!")
    ...
    loss_df = pd.DataFrame(damagebin.astype(int),columns=['landuse','losses']).groupby('landuse').sum()
...
def VectorScanner(landuse,inun_file,curve_path,maxdam_path,landuse_col='landuse',centimeters=False,save=False,**kwargs):
    ...
    if isinstance(inun_file,str):
        ...
        raise ValueError('ERROR: inundation file should be a GeoTiff,  a shapefile, a GeoDataFrame \
    ...
    if isinstance(curve_path, pd.DataFrame):
        ...
        raise ValueError('ERROR: for the vector-based approach we use a pandas DataFrame, not a Numpy Array')
    elif curve_path.endswith('.csv'):
    ...
    if isinstance(maxdam_path, pd.DataFrame):
        ...
        maxdam = dict(zip(pd.read_csv(maxdam_path)[landuse_col],pd.read_csv(maxdam_path)['damage']))
    ...
    for row in tqdm(gdf.itertuples(index=False), total=len(gdf),
                    desc='Get unique shapes', disable=not tqdm_print):
    ...
    tmp_df = pd.DataFrame(unique_df, columns=['depth', landuse_col, 'geometry'])
    ...
    loss_dict = {}
    for x in tqdm(landuse.itertuples(), total=len(landuse),
                  desc='Damage per object', disable=not print_tqdm):
        ...
        if len(inun_levels) == 0:
            ...
            loss_dict[x.Index] = damage,area_flooded,min(inun_levels),max(inun_levels),np.mean(inun_levels)

    tgt_cols = ['tot_dam','area_flooded','min_inun','max_inun','mean_inun']
    loss_df = pd.DataFrame.from_dict(loss_dict,orient='index', columns=tgt_cols)
    loss_gdf = gpd.GeoDataFrame(landuse.merge(loss_df, 
                                              left_index=True,
                                              right_index=True))
    ...
    if save:
        ...
        output_path = check_output_path(kwargs)
        scenario_name = check_scenario_name(kwargs)
        loss_gdf.to_file(p_join(output_path, 'damages_{}.shp'.format(scenario_name)))
