...
def landuse_vector(landuse, color_dict={}, save=False, **kwargs):
    ...
    cmap = LinearSegmentedColormap.from_list(name="landuse", colors=color_scheme_map)
    ...
    if save:
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(output_path, "landuse_{}.png".format(scenario_name)),
            dpi=350,
            bbox_inches="tight",
        )
...
def landuse_raster(landuse_ras, color_dict={}, save=False, **kwargs):
    ...
    with rasterio.open(landuse_ras) as src:
        ...
        cmap = LinearSegmentedColormap.from_list(
            name="landuse", colors=color_scheme_map
        )
...
def inundation_map(inun_map, lu_raster=False, lu_vector=False, save=False, **kwargs):
    ...
    if save:
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(output_path, "inundation_{}.png".format(scenario_name)),
            dpi=350,
            bbox_inches="tight",
        )
...
def damagemap_vector(losses, bins=[], save=False, **kwargs):
    ...
    cmap = LinearSegmentedColormap.from_list(name="damages", colors=color_scheme_map)
    labels = [0, 1, 2, 3, 4]
    if len(bins) == 0:
    ...
    for iter_, item in enumerate(bins):
        if iter_ < len(bins) - 1:
            legend_elements.append(
                Patch(
                    facecolor=color_scheme_map[iter_ + 1],
                    label="{}-{} Euro".format(int(bins[iter_]), int(bins[iter_ + 1])),
                )
            )
    ...
    if save:
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(output_path, "Famagemap_{}.png".format(scenario_name)),
            dpi=350,
            bbox_inches="tight",
        )


def damagemap_raster(
    damagemap, landuse, lu_raster=False, bins=[], save=False, **kwargs
):
    """
    ...
    with rasterio.open(landuse) as src:
        ...
        cmap = LinearSegmentedColormap.from_list(
            name="damages", colors=color_scheme_map
        )
        if len(bins) == 0:
        ...
        for iter_, item in enumerate(bins):
            if iter_ < len(bins) - 1:
                legend_elements.append(
                    Patch(
                        facecolor=color_scheme_map[iter_],
                        label="{}-{} Euro".format(
                            int(bins[iter_]), int(bins[iter_ + 1])
                        ),
                    )
                )
            else:
                legend_elements.append(
                    Patch(
                        facecolor=color_scheme_map[iter_],
                        label="> {} Euro".format(int(bins[iter_])),
                    )
                )
    ...
    if save:
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(output_path, "Damagemap_{}.png".format(scenario_name)),
            dpi=350,
            bbox_inches="tight",
        )
