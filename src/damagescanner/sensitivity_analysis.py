...
def run(landuse_map, inun_map, curve_path, maxdam_path, save=False, **kwargs):
    ...
    param_values = morris.sample(
        problem, 10, num_levels=5, grid_jump=2, local_optimization=True
    )
    ...
    for param_set in tqdm(param_values, total=len(param_values)):
        ...
        loss_df = RasterScanner(
            landuse_map, inundation, curves, maxdam, save=False, in_millions=True
        )
    ...
    angles = numpy.linspace(
        0, 2 * numpy.pi, len(numpy.array(risk_sens.columns)), endpoint=False
    )
